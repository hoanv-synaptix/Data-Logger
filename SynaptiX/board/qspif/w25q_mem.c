 ...
 * @{
 */
extern XSPI_HandleTypeDef hospi1;	///< Quad SPI HAL Instance
/// @}
...
W25Q_STATE W25Q_ReadStatusReg(u8_t *reg_data, u8_t reg_num) {
	...
	if (HAL_XSPI_Command(&hospi1, &com,HAL_XSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
		return W25Q_SPI_ERR;
	}
	if (HAL_XSPI_Receive(&hospi1, reg_data, HAL_XSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
		return W25Q_SPI_ERR;
	}
...
W25Q_STATE W25Q_WriteStatusReg(u8_t reg_data, u8_t reg_num) {
	...
	if (HAL_XSPI_Transmit(&hospi1, &reg_data, HAL_XSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
		return W25Q_SPI_ERR;
	}
...
W25Q_STATE W25Q_ReadRaw(u8_t *buf, u16_t data_len, u32_t rawAddr) {
	...
	if (HAL_XSPI_Receive(&hospi1, buf, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
		return W25Q_SPI_ERR;
...
W25Q_STATE W25Q_ProgramRaw(u8_t *buf, u16_t data_len, u32_t rawAddr) {
	...
	if (HAL_XSPI_Transmit(&hospi1, buf, HAL_XSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK)
		return W25Q_SPI_ERR;
...
W25Q_STATE W25Q_ReadID(u8_t *buf) {
	...
	com.DataDTRMode = HAL_XSPI_DATA_DTR_DISABLE;
	com.DQSMode = HAL_XSPI_DQS_ENABLE;
	com.SIOOMode = HAL_XSPI_SIOO_INST_EVERY_CMD;
	...
	if (HAL_XSPI_Receive(&hospi1, buf, HAL_XSPI_TIMEOUT_DEFAULT_VALUE)
			!= HAL_OK) {
		return W25Q_SPI_ERR;
	}
	return W25Q_OK;
