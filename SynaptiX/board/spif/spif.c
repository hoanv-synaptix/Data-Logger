...
bool SPIF_FindChip(SPIF_HandleTypeDef *Handle)
{
  ...
  do
  {
    ...
    log_info(TAG," - MEMTYPE: 0x%02X", Handle->MemType);
    log_info(TAG," - SIZE: ");
    switch (Handle->Size)
    {
    ...
    log_info(TAG,"SPIF BLOCK CNT: %ld", Handle->BlockCnt);
    log_info(TAG,"SPIF SECTOR CNT: %ld", Handle->SectorCnt);
    log_info(TAG,"SPIF PAGE CNT: %ld", Handle->PageCnt);
    log_info(TAG,"SPIF STATUS1: 0x%02X", SPIF_ReadReg1(Handle));
    log_info(TAG,"SPIF STATUS2: 0x%02X", SPIF_ReadReg2(Handle));
    log_info(TAG,"SPIF STATUS3: 0x%02X", SPIF_ReadReg3(Handle));
    retVal = true;
